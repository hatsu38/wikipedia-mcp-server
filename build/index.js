...
// Wikipedia記事検索ツール
server.tool("search_wikipedia", "Search Wikipedia articles by keyword（キーワードでWikipedia記事を検索する）", {
    ...
    try {
        ...
        const data = await wikipediaRequest(url);
...
server.tool("get_wikipedia_article", "Get detailed information about a Wikipedia article（Wikipedia記事の詳細情報を取得する）", {
    ...
    try {
        ...
        if (page.categories && page.categories.length > 0) {
            result += `**カテゴリ:**\n`;
            page.categories.slice(0, 10).forEach((cat) => {
        ...
        if (page.images && page.images.length > 0) {
            result += `**使用画像 (最初の5件):**\n`;
            page.images.slice(0, 5).forEach((img) => {
                result += `- ${img.title}\n`;
            });
            result += '\n';
        }
...
// ランダム記事取得ツール
server.tool("get_random_wikipedia", "Get random Wikipedia articles（ランダムなWikipedia記事を取得する）", {
    lang: z.string().default("ja").describe("Language code (ja, en, etc.)"),
    ...
    try {
        ...
        const results = data.query.random.map((article) => {
